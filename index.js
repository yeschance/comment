...
function modifyComments(event){ 
    ...
    okBtn.id = "ok"; okBtn.innerHTML="수정"; 
    cancleBtn.id = "cancle"; 
    cancleBtn.innerHTML = "취소"; 
    input.id = "modifyVal"; 
    input.placeholder = "변경할 내용을 입력하세요"; 
    span.id ="inBtn"; span.appendChild(okBtn); 
    ...
    okBtn.addEventListener("click",function(event){ 
        ...
        for(var i=0; i<idOrVoteCountList.length; i++){ 
            if(idOrVoteCountList[i]["commentId"].toString() === mBtn.parentNode.id){ 
                idOrVoteCountList[i]["commentValue"] = input.value; //수정버튼누르면 변경. 
...
function pressBtn(){ 
    ...
    // const bad_words = ["존나", "신발"]
    // let replaceText = "고운말을 씁시다!"

    // const filter = (replaceText) => {
    //     for (let i=0; i<bad_words.length; i++){
    //         if(replaceText.indexOf(bad_words[i]) != -1){
    //             let filter_text = replaceText.replace(bad_words[i], "**");
    //             return console.log(
    //                 `찾았다. \n나쁜말 : ${bad_words[i]} \n변경된 텍스트 : ${filter_text}`
    //             );
    //         }
    //     }
    //     return console.log("나쁜 말이 감지되지 않았습니다.")
    // }
    // filter(replaceText);
}
